...
export default function DocumentationPhotos() {
  ...
  const fetchPhotos = async () => {
    try {
      ...
      const response = await axios.get(
        `https://oneheart.team.com/api/photos/${docId}`
      );
  ...
  const handlePhotoUpload = async (event) => {
    ...
    try {
      setLoading(true);
      await axios.post("https://oneheart.team.com/api/photos", formData, {
  ...
  const handleDeletePhoto = async (photoId) => {
    ...
    if (result.isConfirmed) {
      try {
        await axios.delete(`https://oneheart.team.com/api/photos/${photoId}`);
        fetchPhotos();
          ...
          {photos.map((photo) => (
            <div key={photo._id} className="col-md-4">
              <Card>
                <Card.Img
                  variant="top"
                  src={`https://oneheart.team.com/uploads/documentation/${photo.image}`}
